...
## 撞球问题：在水平光滑无限长的管道两端，有若干相向而行的小球，球的直径和管的直径相同，所有球的质量和速率均相同，球与球之间有一定间距。当两个相向而行的小球相遇时，发生完全弹性碰撞，即为一次碰撞。求下列情况下的碰撞总次数
（1）两端各3个球；（2）一端6个球，一端3个球；（3）一端M个球，一端N个球
（1）9；（2）18；（3）M*N
a)	问题是否理解清楚
b)	给不出正确答案，是否有好的思路
c)	看思路。举例为：对于（1）或（2），可以用穷举法计算，目的是引导推导出M*N的规律
d)	（2）可以从（1）中推导，即将（2）分成两组（1）来考虑
e)	（3）有两种思路，简单的，可以考虑将（2）的分组考虑成18个1*1组合，类推出M*N；另一种思路，当发生一次碰撞时，可以看成两个小球碰撞后交换位置。
...
在一台普通的PC上（默认4字节对齐），struct {char a; float b; short c; char d}这个结构体的大小。
1.	较牛的面试者会问到是几字节对齐的。
2.	普通的面试者会答12字节。
3.	较差的面试者会答16字节。
4.	完全不了解字节对齐的面试者会答8字节。
...
4．	分析：先序中的第一个节点1是二叉树的根，找到该节点在中序中的位置，则1之前的为二叉树的左子树（42），1之后的为二叉树的右子树（536），然后根据先序中的对应子树24和356，递归重建即可。

...
# 3. 登录微信公众号后台，设置服务器配置
```
url: http://mobile.troublehub.com/wechat/callback  // 主要用于处理微信事件回调，比如用户关注、发消息等，本例用不到
token: ***
EncodingAESKey : 随机
```
...
# 6. 登录微信公众号管理后台设置各种域名
* 设置`js接口安全域名` mobile.troublehub.com
* 设置`业务域名` mobile.troublehub.com  // 这样微信在访问该域名时就不会弹出警告信息，影响排版
...
public function authpageAction()
{
  /* 
    ...
    比如需要访问http://mobile.troublehub.com/jsWxPayDemo.html，而该页面需要用到open_id，那么实际的访问地址为：
    http://mobile.troublehub.com/wechat/authpage?redirect=jsWxPayDemo.html
  */
...
因为微信支付需要获取微信用户信息（open_id），所以不能直接跳转相应的支付页面，而是应该先跳转获取微信用户信息的中间页，根据前文，实际的跳转地址为：http://mobile.troublehub.com/wechat/authpage?redirect=jsWxPayDemo.html，可以将该地址配置为微信公众号的菜单。
...
<script>
    $(document).ready(function () {
        ...
        var currUrl = 'http://mobile.troublehub.com/jsWxPayDemo.html';
...
public function paidCallback()
{
  ...
  $chanceChargeInfo = $chanceChargeInfoService->findById($notify->out_trade_no);
    ...
    $chanceInfo = $chanceInfoService->findByCondition(new QueryCondition([
      'user_id' => $chanceChargeInfo->userId,
    ]));

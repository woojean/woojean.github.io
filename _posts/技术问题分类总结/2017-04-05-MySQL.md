...
数据库设计：
表:Message
包含字段：ID（编号）、sender(发送者)、rcver(接受者，为0时表示所有用户)、msgid(消息)、status（状态）
表:MessageText
包含字段: ID(编号)、content(内容)、sendtime(发送时间)

用户登录后，先查询Message里有没有自己未接收过的管理员群发短信，如果有则在Message中新增一条Message，然后查询属于自己的Message。管理员群发短消息时，只在Message里新增一条rcver为0的记录。
...
总之，当进行相等比较的值中可能存在NULL值时，<=>的可移植性更好。
